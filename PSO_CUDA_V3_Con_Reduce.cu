...
__global__ void updateBestGlobal(float* personal_best,
                                    ...
                                    int blocks) {
    // Declarar shared memory para el valor mínimo y su índice
    ...
    if (i < blocks) {
        sdata[tid] = personal_best[i];
        sindex[tid] = bests_index[i];
    } else {
        // Si el índice está fuera de los límites, asignar un valor muy alto para el mínimo
        sdata[tid] = FLT_MAX;
        sindex[tid] = -1;
    }
    ...
    // Realizar la reducción para encontrar el mínimo
    ...
    // Guardar los resultados en la memoria global
    if (tid == 0) {
        *global_best = sdata[0];
        *global_best_index = sindex[0];
    }
}
...
int main() {
    ...
    updateBestGlobal<<<1,512>>>(d_blocks_global_bests,
                                d_blocks_global_bests_index, 
                                d_gBest, 
                                d_gBestIndex, 
                                blocksPerGrid);
    ...
    std::cout << "After UpdateBestGlobal<<1,512>>: " <<  std::endl;
